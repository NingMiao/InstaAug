...
class PreActResFeatureNet_Cifar(nn.Module):
    """
    Pre-activated ResNet network
    """

    def __init__(self, output_layer=[2, -1], output_dims=[], input_size=32, main_layer=3, *args, **kwargs):
        nn.Module.__init__(self)
        
        config_network={"depth":18, "widen_factor":1}
        config_args={"data":{"num_classes":100}, "num_members":1}
    ...
    def scope(self, ranges=False):
        ...
        scopes=[x if x<1.0 else 1.0 for x in scopes]
        if ranges:
...
if __name__=='__main__':
    ...
    feature=net(x)
    print(net.center()[0][0].shape)
    print(net.scope(ranges=True))
