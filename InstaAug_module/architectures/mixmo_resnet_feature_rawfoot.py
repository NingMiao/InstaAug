...
class PreActResFeatureNet_Rawfoot(nn.Module):
    """
    ...
    def _init_output_network(self):
        ...
        for i in range(len(self.output_layer)):
            ...
            if layer in [0,1,2,3]:
                feature=nn.Conv2d(
                self._nChannels[layer], self.output_dims[i], kernel_size=1, stride=1, padding=0, bias=True)
                
            if layer==-1:
                feature=nn.Conv2d(
                self._nChannels[layer], self.output_dims[i], kernel_size=25, stride=1, padding=0, bias=True)
    ...
    def center(self, layer=None):
        
        if layer is None:
            return [self.center(l) for l in self.output_layer]
        ...
        return center_dict[layer]
    
    def scope(self, layer=None):
        
        if layer is None:
            return [self.scope(l) for l in self.output_layer]
